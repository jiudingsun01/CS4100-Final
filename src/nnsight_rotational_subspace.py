...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_names', help='Name of the datasets to be loaded', type=list, required=False, default=HELD_OUT_DATASETS)
    parser.add_argument('--edit_layers', help='Layer for intervention.', type=list, required=False, default=list(range(10, 16)))
    ...
    parser.add_argument('--save_path_root', help='File path to save to', type=str, required=False, default="../multilayer-ins-results")
    parser.add_argument('--intervention_path', help='Path to the trained intervention model', type=str, required=False, default="../multilayer-ins-mix/L10-to-L115/checkpoints/epoch_5")
    ...
    save_path_root = f"{args.save_path_root}/L{str(edit_layers[0])}-to-L{edit_layers[-1]}"
    ...
    if intervention_path is None:
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                loss = calculate_loss(counterfactual_outputs, inputs["base_labels"], subspace_proj, dimension_weights, model.config.vocab_size)
                loss_str = round(loss.item(), 2)
                
                log_dict = {'loss': loss_str, 'acc': eval_metrics["accuracy"], 'sparsity': compute_rotation_mask_sparsity(subspace_proj)}
            ...
            if evaluate_per_epoch:
                ...
                fs_shuffled_acc = evaluate_w_subspace_intervention_multilayer(model, subspace_proj, fs_eval_dataloader, edit_layers, device=model.device, generate_output=generate_output)
                epoch_training_log['fs_shuffled_with_intervention_accuracy'] = fs_shuffled_acc["accuracy"]
                
                zs_intervention_acc = evaluate_w_subspace_intervention_multilayer(model, subspace_proj, zs_eval_dataloader, edit_layers, device=model.device, generate_output=generate_output)
            ...
            if epoch % checkpoint_every_epoch == 0:
                ...
                torch.save(subspace_proj.state_dict(), f"{ckpt_path}/model.model_state_dict.bin")
    ...
    torch.save(subspace_proj.state_dict(), f"{save_path_root}/intervention_model/model.model_state_dict.bin")
    print("Done!")
