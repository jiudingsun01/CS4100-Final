...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', help='Name of model to be loaded', type=str, required=False, default='/data/jiuding_sun/function_vectors/flan-llama-7b')
    ...
    parser.add_argument('--user_filter_set', help='Whether filter inference set with only the correct examples', type=bool, required=False, default=False)
    ...
    device = args.device
    user_filter_set = args.user_filter_set
    
    prefixes = load_prefixes_or_separators(args.prefixes)
    separators = load_prefixes_or_separators(args.separators)
    
    counterfactual_prefixes = {"input": prefixes["input"], "output": prefixes["output"], "instructions": ""}
    counterfactual_separators = {"input": separators["input"], "output": separators["output"], "instructions": ""}
    ...
    for seed in seeds:
        ...
        mean_activations = get_mean_layer_activations(dataset, model=model, model_config=model_config, tokenizer=tokenizer, prefixes=prefixes, separators=separators,
        ...
        fs_results = n_shot_eval_no_intervention(dataset, n_shots, model, model_config, tokenizer, prefixes=prefixes, separators=separators)
        filter_set = np.where(np.array(fs_results['clean_rank_list']) == 0)[0] if user_filter_set else None

        zs_res = {}
        fss_res = {}
        for i in range(model_config['n_layers']):
            zs_res[i] = n_shot_eval(dataset, mean_activations[i].unsqueeze(0), i, 0, model, model_config, tokenizer,  prefixes=counterfactual_prefixes, separators=counterfactual_separators, filter_set=filter_set)
            fss_res[i] = n_shot_eval(dataset, mean_activations[i].unsqueeze(0), i, 10, model, model_config, tokenizer, prefixes=counterfactual_prefixes, separators=counterfactual_separators, filter_set=filter_set, shuffle_labels=True)
