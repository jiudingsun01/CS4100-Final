...
if __name__ == "__main__":
    ...
    parser.add_argument('--edit_layer', help='Layer for intervention.', type=int, required=False, default=14)
    parser.add_argument('--model_name', help='Name of model to be loaded', type=str, required=False, default="/data/public_models/mistral/mistral-7b-instruct-v0.1")
    parser.add_argument('--root_data_dir', help='Root directory of data files', type=str, required=False, default='../dataset_files')
    parser.add_argument('--save_path_root', help='File path to save to', type=str, required=False, default="../mistral_debug")
    parser.add_argument('--intervention_path', help='Path to the trained intervention model', type=str, required=False, default=None)
    ...
    parser.add_argument('--training_method',type=str, required=False, default='zero_shot', choices=['noninformative', 'zero_shot', 'both'])
    ...
    parser.add_argument('--epochs', help='Batch size of inference and training intervention', type=int, required=False, default=5)
    parser.add_argument('--dimension_weights', help='weights assign to the dimension loss', type=float, required=False, default=2)
    ...
    parser.add_argument('--checkpoint_every_epoch', type=int, default=5)
    ...
    """if os.path.exists(os.path.join(save_path_root, "results.json")):
        print("This experiment has already been run. Exiting...")
        exit()"""
    ...
    def calculate_loss(logits, labels):
        ...
        for k, v in intervenable.interventions.items():
            boundary_loss = dimension_weights * v[0].intervention_boundaries.sum()
        loss += boundary_loss
    ...
    if not os.path.exists(save_path_root):
        pathlib.Path(save_path_root).mkdir(parents=True, exist_ok=True)
    ...
    if training_method == "both":
        noninformative_dataset = process_dataset(dataset, model_config, tokenizer, n_shots, "train", prefixes, separators, n_trials=n_trials, ablation_method="noninformative", draw_source_from_split=False)
        zeroshot_dataset = process_dataset(dataset, model_config, tokenizer, n_shots, "train", prefixes, separators, n_trials=n_trials, ablation_method="zero_shot", draw_source_from_split=False)
        ...
        train_dataloader = process_dataloader(dataset, model_config, tokenizer, batch_size, n_shots, "train", prefixes, separators, intervention_collate_fn, n_trials=n_trials, ablation_method=training_method, draw_source_from_split=False)
    ...
    eval_dict = evaluate(intervenable, eval_no_intervention_dataloader, device=model.device, intervene=False, corrupt=False, generate_output=generate_output)
    ...
    if intervention_path is None:
        ...
        for epoch in train_iterator:
            ...
            if evaluate_per_epoch:
                ...
                fs_shuffled_acc = evaluate(intervenable, fs_eval_dataloader, device=model.device, intervene=True, generate_output=False)
                epoch_training_log['fs_shuffled_with_intervention_accuracy'] = fs_shuffled_acc["accuracy"]
                
                zs_intervention_acc = evaluate(intervenable, zs_eval_dataloader, device=model.device, intervene=True, generate_output=False)
    ...
    fs_shuffled_acc = evaluate(intervenable, fs_eval_dataloader, device=model.device, intervene=True, generate_output=generate_output)
    results['fs_shuffled_with_intervention_accuracy'] = fs_shuffled_acc["accuracy"]
    
    fs_shuffled_no_intervention_acc = evaluate(intervenable, fs_eval_dataloader, device=model.device, intervene=False, corrupt=True, generate_output=generate_output)
    results['fs_shuffled_no_intervention_accuracy'] = fs_shuffled_no_intervention_acc["accuracy"]
    
    zs_intervention_acc = evaluate(intervenable, zs_eval_dataloader, device=model.device, intervene=True, generate_output=generate_output)
    results['zs_with_intervention_accuracy'] = zs_intervention_acc["accuracy"]
        
    zs_no_intervention_acc = evaluate(intervenable, zs_eval_dataloader, device=model.device, intervene=False, corrupt=True, generate_output=generate_output)
