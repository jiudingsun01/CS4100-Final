...
def activation_replacement_per_class_intervention(prompt_data, avg_activations, dummy_labels, model, model_config, tokenizer, last_token_only=True, instruction_ablation="unknown"):
    """
    ...
    is_llama = 'llama' in model_config['name_or_path'] or 'mistral' in model_config['name_or_path']
    if is_llama:
...
def compute_indirect_effect(dataset, mean_activations, model, model_config, tokenizer, n_shots=10, n_trials=25, last_token_only=True, prefixes=None, separators=None, filter_set=None, instruction_ablation="unknown"):
    """
