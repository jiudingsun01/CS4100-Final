...
if __name__ == "__main__":
    ...
    parser.add_argument('--no_instruction', help='Whether filter inference set with only the correct examples', type=bool, required=False, default=True)
    ...
    if os.path.exists(fs_results_file_name):
        ...
        set_seed(seed+42)
        fs_results_validation = n_shot_eval_no_intervention(dataset=dataset, n_shots=n_shots, model=model, model_config=model_config, tokenizer=tokenizer, prefixes=prefixes, separators=separators, compute_ppl=False,
