        ...
        if isinstance(word, list):
            for j, (word2,label2) in enumerate(zip(word,label)):
                ...
                if word2.startswith(' '):  
                    ...
                    n_tokens = tokenizer(word2, return_length=True).length
            ...
            prompt_builder += word
            post = tokenizer(prompt_builder, return_length=True).length
            n_tokens = tokenizer(word, return_length=True).length
...
def tokenize_labels(sentence_parts, text_labels, tokenizer):
    ...
    is_llama = 'llama' in tokenizer.name_or_path or 'alpaca' in tokenizer.name_or_path or 'mistral' in tokenizer.name_or_path
...
def get_dummy_token_labels(n_icl_examples, tokenizer, prefixes=None, separators=None):
    ...
    """
    is_llama = 'llama' in tokenizer.name_or_path or 'mistral' in tokenizer.name_or_path
...
def load_prefixes_or_separators(prefixes_or_separators):
    ...
    if type(prefixes_or_separators) == dict:
    ...
    elif type(prefixes_or_separators) == list:
        for prefix_or_separator in prefixes_or_separators:
            assert "input" in prefix_or_separator.keys(), "Error! prefixes_or_separators must contain 'input' key"
            assert "output" in prefix_or_separator.keys(), "Error! prefixes_or_separators must contain 'output' key"
            assert "instructions" in prefix_or_separator.keys(), "Error! prefixes_or_separators must contain 'instructions' key"
        return prefixes_or_separators
        ...
        if type(prefixes_or_separators) == dict:
            ...
            raise ValueError(f"Error! prefixes_or_separators={prefixes_or_separators} is not a valid type. Expected one of: [dict, list]")
    else:
        raise ValueError(f"Error! prefixes_or_separators={prefixes_or_separators} is not a valid type. Expected one of: [dict, str]")
    
def load_task_specific_prefixes_or_separators(dataset_names, root_data_dir, is_prefixes=False):
...
def load_alpaca_input_output(instruction, input, output):
    if input == "":
